...
def naiveSoftmaxLossAndGradient(
    ...
    gradOutsideVecs = np.dot(y_hat[:, np.newaxis], centerWordVec[np.newaxis, :])
...
def negSamplingLossAndGradient(
    ...
    for k in range(K):
        ...
        loss += -np.log(z)
        gradCenterVec += -1.0 * outsideVectors[samp] * (z - 1.0)
        gradOutsideVecs[samp] += -1.0 * centerWordVec * (z - 1.0)
...
def skipgram(currentCenterWord, windowSize, outsideWords, word2Ind,
    ...
    for outsideWord in outsideWords:
        ...
        currLoss, gradCenter, gradOutside = word2vecLossAndGradient(centerWordVec, outsideWordIdx, outsideVectors,dataset)
        loss += currLoss
